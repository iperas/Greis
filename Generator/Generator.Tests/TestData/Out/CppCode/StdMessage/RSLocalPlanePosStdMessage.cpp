#include "RSLocalPlanePosStdMessage.h"
#include <cassert>
...
namespace Greis
{
    RSLocalPlanePosStdMessage::RSLocalPlanePosStdMessage( const char* pc_message, int p_length ) 
    ...
    RSLocalPlanePosStdMessage::RSLocalPlanePosStdMessage( const std::string& p_id, int p_size ) 
        : _id(p_id), _bodySize(p_size - HeadSize())
    {
        _isCorrect = true;
    }

    std::string RSLocalPlanePosStdMessage::ToString() const
    {
        return toString("RSLocalPlanePosStdMessage");
    }
    
    bool RSLocalPlanePosStdMessage::Validate() const
    {
    ...
    void RSLocalPlanePosStdMessage::RecalculateChecksum()
    {
        if (!_isCorrect)
        {
            return;
    ...
    QByteArray RSLocalPlanePosStdMessage::ToByteArray() const
    {
        QByteArray result;
        if (!_isCorrect)
        {

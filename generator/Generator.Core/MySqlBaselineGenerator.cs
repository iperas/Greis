...
namespace Generator.Core
{
    public class MySqlBaselineGenerator
    {
        ...
        private readonly SqlTableNameFactory tableNames;
        ...
        public void Generate(string outFileName)
        {
            ...
            var useDatabase = this.GenerateUseDatabase();
            var tableDrop = this.GenerateTableDrop();
            var tableCreation = this.GenerateTableCreation();
            var fillup = this.GenerateInitialFillup();

            // Apply it and save as new baseline file
            var encoding = Encoding.UTF8;
            var baseline = File.ReadAllText(this.templateFilename, encoding);
        ...
        {
            return string.Format("USE `{0}`;", this.databaseName);
        }
        ...
        private string GenerateTableCreation()
        {
            ...
            foreach (var ct in this.metaInfo.CustomTypes)
            {
                ...
                var query = string.Format(queryCtFmt, ct.Name, this.tableNames.GetName(ct), colDefs);
            ...
            foreach (var msg in this.metaInfo.StandardMessages)
            {
                ...
                var query = string.Format(queryMsgFmt, msg.Name, msg.Title.Replace("\r", "").Replace('\n', ' '),
                                          this.tableNames.GetName(msg), colDefs);
                tableCreationSb.Append(query);
            }
        ...
        private string GenerateTableDrop()
        {
            ...
            foreach (var msg in this.metaInfo.StandardMessages)
            {
                tableDropSb.AppendFormat("DROP TABLE IF EXISTS `{0}`;\r\n", this.tableNames.GetName(msg));
            }
            foreach (var ct in this.metaInfo.CustomTypes)
            {
                tableDropSb.AppendFormat("DROP TABLE IF EXISTS `{0}`;\r\n", this.tableNames.GetName(ct));
            }
        ...
        private string GenerateInitialFillup()
        {
            ...
            foreach (var ct in this.metaInfo.CustomTypes)
            {
                ...
                var ctMetaInsert = string.Format("({3}, '{0}', {1}, '{2}')", ct.Name, ct.Size,
                                                 this.tableNames.GetName(ct), customTypeId);
            ...
            foreach (var message in this.metaInfo.StandardMessages)
            {
                ...
                var messageMetaInsert = string.Format("({0}, '{1}', '{2}', {3}, {4}, {5}, {6}, '{7}')", messageId,
                                                      ...
                                                      (int)message.Type, this.tableNames.GetName(message));
